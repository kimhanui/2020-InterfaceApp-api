...
public interface AttendanceRepository extends JpaRepository<Attendance, Long> {
    ...
    void deleteAllByMeeting_Id(@Param("meeting_id") Long meeting_id);

    @Query("select a from Attendance a where a.meeting.id in :meeting_id")
    List<Attendance> findAllByMeeting_Id(@Param("meeting_id") Long meeting_id);

    @Query("select a from Attendance a where a.participant.id in :participant_id")
    List<Attendance> findAllByParticipant_Id(@Param("participant_id") Long participant_id);
}
